...
class MovingAverageBatchNormalization(function_node.FunctionNode):
    mean = None
    inv_std = None

    def __init__(self, eps=2e-5, mean=None, var=None, decay=0.99, axis=None):
    ...
    def forward(self, inputs):
        ...
        if xp is numpy:
            self.inv_std = numpy.reciprocal(numpy.sqrt(
                self.running_var + self.eps, dtype=x.dtype))
        else:
            self.inv_std = cuda.cupyx.rsqrt(self.running_var + self.eps)
        y = _apply_bn_fwd(xp, x, self.running_mean[expander],
    ...
    def backward(self, indexes, grad_outputs):
        ...
        f = BatchNormalizationGrad(
            ...
            self.running_mean, None, self.inv_std, self.key_axis)
        return f(x, gamma, gy)
...
def moving_average_batch_normalization(x, gamma, beta, **kwargs):
    ...
    return MovingAverageBatchNormalization(eps, running_mean, running_var, decay,
